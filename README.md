...
```bash
event-driven-app$ make build
event-driven-app$ make deploy.guided
```
...
```bash
event-driven-app$ make run
event-driven-app$ curl http://localhost:3000/hello
```
...
```bash
event-driven-app$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
event-driven-app$ make dev
event-driven-app$ make test
```
...
```bash
event-driven-app$ aws cloudformation delete-stack --stack-name event-driven-app
```
